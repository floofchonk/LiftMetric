...
(function () {
  ...
  function handleVisualEditorMouseMove(event) {
    ...
    // If the resolved element is sitting over an <img> (gradient overlay,
    // figcaption inside a <figure>, picture wrapper, etc.), retarget to the
    // underlying <img> so hover lights up exactly the image the user is
    // pointing at — not all 9 overlays/captions sharing a .map() shipperId.
    if (elementWithId && elementWithId.tagName?.toLowerCase() !== "img") {
      const retargeted = retargetToImageIfApplicable(elementWithId);
      if (retargeted !== elementWithId && retargeted.tagName?.toLowerCase() === "img") {
        elementWithId = retargeted;
        const retargetedId = retargeted.getAttribute("data-shipper-id");
        if (retargetedId) shipperId = retargetedId;
      }
    }

    // Hovering a text element layered over an image (e.g. a heading
    // absolutely positioned on a hero image) — retarget to the image so
    // the hover highlight matches what the click would select. Same z-
    // stack check as in handleVisualEditorClick to keep behaviour
    // consistent.
    if (
      elementWithId &&
      elementWithId.tagName?.toLowerCase() !== "img" &&
      typeof event.clientX === "number" &&
      typeof event.clientY === "number"
    ) {
      const stack = document.elementsFromPoint(event.clientX, event.clientY);
      const imageBelow = stack.find((node) => {
        if (node === elementWithId) return false;
      ...
      if (imageBelow) {
        const targetStyle = window.getComputedStyle(elementWithId);
        const targetTag = elementWithId.tagName.toLowerCase();
        const isTextLike =
          /^(h[1-6]|p|span|li|figcaption|label|small|strong|em|b|i)$/.test(
            targetTag,
          ) ||
          (targetTag === "div" && (elementWithId.textContent?.trim().length ?? 0) > 0);
        ...
        if (
          isTextLike &&
          !isInteractiveElement(elementWithId) &&
          !targetHasOwnBgImage &&
          !targetHasOwnBgColor
        ) {
          elementWithId = imageBelow;
          const retargetedId =
            imageBelow.getAttribute?.("data-shipper-id");
          if (retargetedId) shipperId = retargetedId;
        }
      }
    }

    // Hover always highlights only the element under the cursor — even
    // for repeated sets (cards/buttons/images rendered from a .map()
    // share a shipperId, but the user is pointing at one specific
    // instance). Multi-instance treatment, if any, only happens on click
    // where the user has made a deliberate selection.
  ...
  function handleVisualEditorClick(event) {
    ...
    if (shipperId) {
      ...
      // Images show BOTH blue dashed inner border + pink solid outer outline.
      // (Pink renders on the outside so it visually sits above the blue.)
      // Non-images just show blue dashed.
      const applySelectionStyle = (el) => {
        el.style.boxShadow = "none";
        el.style.borderRadius = "9.5px";
        el.style.background = "transparent";
        if (isImageLike) {
          el.style.border = `2px dashed ${CONFIG.HIGHLIGHT_COLOR}`;
      ...
      if (elementsWithSameId.length > 1) {
        hideRepeatedHoverOverlays();
        if (isImageLike) {
          // For images, only highlight the actually-clicked instance.
          // Other repeated images shouldn't get the pink/blue ring.
          applySelectionStyle(visualEditorState.highlightOverlay);
          updateOverlay(visualEditorState.highlightOverlay, selectedElement);
        } else {
          // Non-image repeated elements: highlight all instances (existing behavior).
          elementsWithSameId.forEach((element, index) => {
            const overlay = getOrCreateRepeatedHoverOverlay(index);
            applySelectionStyle(overlay);
        ...
        // Single element - show overlay positioned on the actual selected element
        hideRepeatedHoverOverlays();
        applySelectionStyle(visualEditorState.highlightOverlay);
        updateOverlay(visualEditorState.highlightOverlay, selectedElement);
      }
    } else {
      // No shipperId - show overlay positioned on the actual selected element
      hideRepeatedHoverOverlays();
      visualEditorState.highlightOverlay.style.boxShadow = "none";
      visualEditorState.highlightOverlay.style.borderRadius = "9.5px";
      visualEditorState.highlightOverlay.style.background = "transparent";
      if (isImageLike) {
        visualEditorState.highlightOverlay.style.border = `2px dashed ${CONFIG.HIGHLIGHT_COLOR}`;
        visualEditorState.highlightOverlay.style.outline = "2px solid #EC4899";
        visualEditorState.highlightOverlay.style.outlineOffset = "0px";
      } else {
        visualEditorState.highlightOverlay.style.border = `2px dashed ${CONFIG.HIGHLIGHT_COLOR}`;
        visualEditorState.highlightOverlay.style.outline = "none";
      }
      updateOverlay(visualEditorState.highlightOverlay, selectedElement);
    }
      ...
      if (elementsWithSameId.length > 1) {
        ...
        if (isSelected) {
          ...
          elementsWithSameId.forEach((el, index) => {
            ...
            overlay.style.borderRadius = "9.5px";
